...
def train(env, input_dims, action_space,
    ...
    for i_episode in tqdm.tqdm(range(1, max_episodes + 1), desc='Episodes: '):
        state = env.reset()
        for t in tqdm.tqdm(range(max_timesteps), desc='Timesteps: ', position=1, leave=False):
            time_step += 1
        ...
        if i_episode % save_interval == 0:
            ...
            #---------------------
            tqdm.tqdm.write('Checkpoint saved')

        if i_episode % print_interval == 0:
            running_reward = int((running_reward / print_interval))
            tqdm.tqdm.write('Episode {} \t Avg reward: {}'.format(i_episode, running_reward))
            running_reward = 0

...
def gt_tg():
    ...
    for z in certainty_bar:
        ...
        for i, (algorithm, args, kwargs) in enumerate(rl_algos):
            (gt_average_regret[i], gt_percents_optimal[i], gt_selects_mean[i]), (rl_average_regret[i], rl_percents_optimal[i]) = train_and_evaluate_gt(bandit, num_steps, num_runs, algorithm, gt_algo, certainty = z, regret=True, trial=False, *args, **kwargs)
            tqdm.write(f"gt_selects_mean for {algorithm}: {gt_selects_mean[i]}")
            tqdm.write(f"gt_selects_std for {algorithm}: {gt_selects_std[i]}")
        folder = './results/EXP3IXrl/'
        if not os.path.exists(folder):
            os.makedirs(folder)
        np.save(folder + f'gt_average_regret_{z}.npy', gt_average_regret)
        np.save(folder + f'gt_percent_optimal_{z}.npy', gt_percents_optimal)
        np.save(folder + f'rl_average_regret_{z}.npy', rl_average_regret)
        ...
        plt.savefig(f'{folder}EXP3IXrl-{z}.png')
        plt.close()
    ...
    for _ in tqdm(range(num_runs), desc=f'Running {str(algorithm)}'):
        ...
        if regret:
            if not trial:
                ...
                # print(type(cumulative_rewards), type(cumulative_rewards[0]), cumulative_rewards[0].shape)
                tmp =np.vstack(cumulative_rewards)
                # print(tmp.shape)
...
def run_exp3():
    ...
    plt.savefig(folder + f"EXP3IX_{num_steps}.pdf", format="pdf", bbox_inches="tight")
    plt.close()
...
def epsilon_greedy():
    ...
    for i, epsilon in enumerate(epsilons):
        t_a, t_p = train_and_evaluate(bandit=bandit, num_steps=num_steps, num_runs=num_runs, algorithm=EpsilonGreedy, regret=False, epsilon=epsilon)
        # t_a, t_p = train_and_evaluate(
        #     bandit, num_steps, num_runs, EpsilonGreedy, epsilon)
        average_rewards[i] = t_a
        percents_optimal[i] = t_p
